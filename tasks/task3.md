...
답 : 버그를 유발한 커밋 id 25ede83

버그 수정 :
411.번재 라인에  this.delayBeforeBullet = 10; 이 명령어가 삭제가 되어 총알이 계속 나오는 것이고 
이 명령어를 추가하면 스페이스를 계속 누르더라도 계속해서 총알이 나오지 않아 버그를 해결할 수 있다
그리고 총알이 더 느리게 발사되게 하고 싶으면 this.delayBeforeBullet의 값을 10 이상을 주게 되면 더 느리게 나오게 된다.
